...
def _save_history(history_path: Path, entries: list[dict[str, Any]]) -> None:
    """Persist history without exposing a partially-written JSON file."""
...
def delete_history_entry(history_path: Path, job_id: str) -> bool:
    with _history_lock:
        ...
        if len(remaining) == len(entries):
            return False
        _save_history(history_path, remaining)
        return True
