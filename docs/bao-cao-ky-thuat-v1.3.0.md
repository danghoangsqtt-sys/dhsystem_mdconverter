# Báo cáo kỹ thuật — Mark Tini Editor v1.3.0

Ngày lập báo cáo: 20/08/2026
Phiên bản: 1.3.0 (đã sửa lỗi trích xuất vùng PDF trả về placeholder thay vì văn bản thật, đơn giản hóa panel kết quả dịch, thêm sidebar thu gọn/mở rộng và kéo giãn tự do)
...
Với input dạng ảnh đơn lẻ (một trang hoặc một vùng đã cắt — xem mục 3.4), model layout của docling đôi khi phân loại nhầm toàn bộ ảnh thành một cluster loại "Picture" duy nhất do thiếu ngữ cảnh trang đầy đủ; khi đó `export_to_markdown()` xuất ra placeholder `<!-- image -->` thay vì nội dung. Vì vậy pipeline ảnh đọc thẳng `result.document.texts` (danh sách phần tử văn bản docling đã nhận dạng được, không phụ thuộc việc phân loại cluster có đúng hay không) thay vì đi qua bước export chuẩn — đây là điểm khác biệt duy nhất giữa nhánh xử lý ảnh và nhánh PDF/DOCX trong lưu đồ trên.
...
**Đã sửa trong v1.3.0:** trước đây bước "Đọc thẳng result.document.texts" ở trên thực chất đi qua `export_to_markdown()` như nhánh PDF/DOCX, khiến vùng cắt nhỏ (đặc biệt vùng chỉ chứa văn bản, không có layout phức tạp) thường bị model layout phân loại nhầm thành "Picture" và trả về placeholder `<!-- image -->` thay vì chữ thật — kéo theo cả nút "Tìm trên web" tìm nhầm chuỗi placeholder. Xem mục 3.2 để biết chi tiết nguyên nhân và cách sửa; đã verify lại bằng vùng chọn thật (cả nhỏ lẫn lớn) qua app Electron thật, không còn placeholder.
...
**Thay đổi trong v1.3.0:** panel kết quả (`TranslationPanel`) trước đây hiển thị văn bản gốc và bản dịch xếp chồng nhau; nay chỉ hiển thị bản dịch (kèm badge chiều dịch và lĩnh vực thuật ngữ nếu có) — văn bản gốc đã hiển thị sẵn ngay phía trên (chính là đoạn người dùng vừa bôi đen trong bản xem trước) nên không cần lặp lại, giúp panel gọn hơn.
...
Toàn bộ trạng thái Sidebar (độ rộng, đang thu gọn hay không) chỉ tồn tại ở tầng giao diện (`localStorage` của renderer), không liên quan tới backend hay dữ liệu tài liệu — mục đích thuần túy là tùy biến không gian làm việc theo màn hình/thói quen từng người, ví dụ thu gọn để có thêm chỗ cho khung xem PDF hai cột khi làm việc với tài liệu nhiều hình.
...
Chọn một vùng bất kỳ trên trang PDF (ví dụ một hình/bảng nhỏ) để OCR riêng vùng đó, không phải chuyển cả file. Kết quả hiện ở panel riêng, có thể sửa văn bản, tìm kiếm trên web, hoặc chèn thẳng vào tài liệu Markdown đang soạn (xem lưu đồ mục 3.4). Từ v1.3.0, cả vùng chọn nhỏ lẫn lớn đều trả về đúng văn bản nhận dạng được, không còn placeholder `<!-- image -->`.
...
- Kết quả hiển thị gọn — chỉ bản dịch kèm badge chiều dịch/lĩnh vực, không lặp lại văn bản gốc (đã bôi đen sẵn ở trên).
...
- **Mới trong v1.3.0:** Sidebar bên trái thu gọn được (còn dải icon 56px) để nhường chỗ cho khung soạn thảo/xem PDF, hoặc kéo giãn tự do 200-420px theo ý muốn — trạng thái được nhớ qua các lần mở app (xem lưu đồ mục 3.9).
...
Hướng dẫn sử dụng đầy đủ dành cho người dùng cuối được lưu riêng tại [`huong-dan-su-dung-v1.3.0.md`](huong-dan-su-dung-v1.3.0.md). Tóm tắt nhanh các thao tác chính:
...
9. **Thu gọn/mở rộng Sidebar:** nút mũi tên ở đầu Sidebar để thu gọn còn dải icon, hoặc kéo dải viền phải để chỉnh độ rộng tự do.
...
- **Mới trong v1.3.0:** sửa lỗi trích xuất vùng PDF trả về placeholder `<!-- image -->` thay vì văn bản thật (xem mục 3.2/3.4) — verify bằng cả vùng chọn nhỏ và lớn qua app Electron thật, không còn dựa vào đọc code hay gọi Python trực tiếp.
- **Mới trong v1.3.0:** đơn giản hóa panel kết quả dịch — bỏ khung lặp lại văn bản gốc, chỉ giữ bản dịch.
- **Mới trong v1.3.0:** thêm Sidebar thu gọn/mở rộng và kéo giãn tự do (200-420px), trạng thái nhớ qua các lần mở app — verify qua Playwright điều khiển app Electron thật (không chỉ kiểm tra kiểu dữ liệu TypeScript).
- Đã build lại installer và cài đặt/khởi chạy thành công trên máy phát triển sau các thay đổi trên (không phải máy sạch — xem mục còn lại bên dưới).
